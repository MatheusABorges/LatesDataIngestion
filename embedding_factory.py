...
class EmbeddingHandler:
    def __init__(self, provider: str, model_name: str, api_key: str = None, is_ingestion: bool = True, device: str = "cpu"):
        """
        Inicializa o handler, carrega o modelo e calcula sua dimensão imediatamente.
        
        Args:
            provider: 'huggingface_local', 'google', 'openai', etc.
            model_name: Caminho ou nome do modelo.
            api_key: Chave de API (opcional, dependendo do provider).
            is_ingestion: Otimização para indexação (True) ou busca (False).
        """
        ...
        # 1. Instancia o modelo (Lógica da Factory)
        self._model_instance = self._create_model()
        
        # 2. Calcula a dimensão (Auto-Introspecção)
        self._dimension = self._calculate_dimension()
        
        print(f"Embedding Handler pronto: {self.provider} | Dimensão: {self._dimension}")

    @property
    def model(self) -> Embeddings:
        """Getter para acessar a instância do LangChain"""
        return self._model_instance

    @property
    def dimension(self) -> int:
        """Getter para acessar a dimensão calculada"""
        return self._dimension

    def _create_model(self) -> Embeddings:
        """Método interno para instanciar o provedor correto"""
        
        # --- HUGGING FACE LOCAL ---
        if self.provider == "huggingface_local":
            from langchain_huggingface import HuggingFaceEmbeddings
            
            # Lógica de verificação/download
